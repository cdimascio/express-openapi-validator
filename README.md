...
- `{ ... }` (**default**) Sets the derefence mode e.g. `bundle`
  ...
  **mode**:
  ...
  ```
- `false` - disable the `$refParser` and do not dereference the api spec. **Warning**: Use this option if and only if you are certain that the api spec is already dereferenced and valid. If it is not, you may encounter unexpected behavior.
